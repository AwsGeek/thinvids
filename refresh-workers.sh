...
for node in "${nodes[@]}"; do
  (
    ...
    scp docker-compose-worker.yml "${ssh_user}@${ip}":docker-compose.yml
    ssh -t "${ssh_user}@${ip}" 'sudo docker compose pull && sudo docker compose down && sudo docker compose up -d worker'
