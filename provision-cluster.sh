...
for node in $nodes; do
  (
    ...
    echo "Setting up $node..." | tee "$log_file"

    ssh "$ssh_user@$node" bash -s <<EOF 2>&1 | tee -a "$log_file"
...
EOF
    ...
    echo "Finished provisioning $node" | tee -a "$log_file"
  ) &
done
...
brick_list=""
for node in $nodes; do
  brick_list="$brick_list $node:$GLUSTER_BRICK"
done
