...
def segment_video(job_id, file_path, filename):
    try:
        ...
        job_dir = os.path.join('/tmp', f"job_{job_id}")
        ...
        while True:
            ...
            if match:
                ...
                #    queued_chunks.add(previous_chunk_index)
                    logger.info(f"VEM [{job_id}] Segmented and queued chunk {previous_chunk_index}")
...
def stitch_video(job_id):
    try:
        ...
        try:
            os.makedirs(final_dir, exist_ok=True)
            shutil.move(local_output_path, final_path)  # use shutils because this is a cross device move
